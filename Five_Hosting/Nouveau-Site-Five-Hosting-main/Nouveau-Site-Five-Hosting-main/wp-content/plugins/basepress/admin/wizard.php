	...
	class Basepress_Wizard{
		...
		public function admin_notices(){
			...
			<div class="notice notice-error">
				<p>
					<?php _e( 'Your Knowledge Base is not set up yet. Use the Setup Wizard to get started.', 'basepress' ); ?>
					<a class="button button-primary" href="<?php menu_page_url( 'basepress_wizard' ); ?>"><?php _e( 'Start Wizard', 'basepress' ); ?></a>
					<a class="button" href="<?php echo $disable_wizard_url; ?>"><?php _e( 'Skip Wizard', 'basepress' ); ?></a>
				</p>
			</div>
			<?php
		}
			...
			<div class="basepress-wizard-box">
				...
				<form id="basepress-wizard-form">
					...
					<div class="basepress-wizard-step">
						<div class="basepress-wizard-item">
							...
							<select name="menu" id="menu">
								<option value=""><?php _e( 'Select Menu', 'basepress' ); ?></option>
					...
					<div class="basepress-wizard-step">
						...
						<div class="basepress-wizard-item">
							<label for="search-min-chars"><?php _e( 'Minimum word length for searches', 'basepress' ); ?></label><br>
							<input type="number" name="search_min_chars" id="search-min-chars" value="3" min="1" max="9">
							<p class="wizard-description"><?php _e( 'Any search term shorter than this value will be omitted from the search.', 'basepress' ); ?></p>
						</div>
						<div class="basepress-wizard-item">
							<input type="checkbox" name="disable_word_boundary" id="disable-word-boundary" value="1">
							<label for="disable-word-boundary"><?php _e( 'Use wild card before search terms', 'basepress' ); ?></label>
				...
				$disable_wizard_url = add_query_arg( 'basepress_skip_wizard', 'true' );
				?>
				<a id="basepress-skip-wizard" href="<?php echo $disable_wizard_url; ?>"><?php _e( 'Skip Wizard', 'basepress' ); ?></a>
			</div>
			<?php
		}
		...
		public function basepress_wizard_proceed(){
			...
			$menu_id = isset( $settings['menu'] ) && ! empty( $settings['menu'] )? $settings['menu'] : false;
			$menu_item = null;
			...
			$search_min_chars = isset( $settings['search_min_chars'] ) ? (int)$settings['search_min_chars'] : 3;
		...
		private function get_response( $post_id, $menu_item, $posts_with_shortcode ){
			...
			if( is_wp_error( $post_id ) || is_wp_error( $menu_item ) ){
				...
				if( $kb_link ){
					$response .= '<p><a class="basepress-wizard-view-kb" href="' . $kb_link .'">' . __( 'View Knowledge Base', 'basepress' ) . '</a></p>';
				}
				$response .= '</h2>';
