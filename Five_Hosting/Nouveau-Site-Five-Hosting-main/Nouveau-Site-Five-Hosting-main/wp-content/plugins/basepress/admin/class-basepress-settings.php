	...
	class BasePress_Settings {
		...
		*/
		public function settings_init() {
			...
			//Add theme settings
			add_settings_section( 'basepress_theme_settings', __( 'Appearance', 'basepress' ), '', 'basepress' );

			//Add breadcrumbs settings
			add_settings_section( 'basepress_breadcrumbs_settings', __( 'Breadcrumbs', 'basepress' ), '', 'basepress' );
			...
			//Add article comments settings
			add_settings_section( 'basepress_settings_import_export', __( 'Import/Export', 'basepress' ), '', 'basepress' );
			...
			add_settings_field( 'article_permalink_structure', __( 'Articles permalink structure', 'basepress' ), array( $this, 'article_permalink_structure_render' ), 'basepress', 'basepress_general_settings' );
			add_settings_field( 'section_permalink_structure', __( 'Sections permalink structure', 'basepress' ), array( $this, 'section_permalink_structure_render' ), 'basepress', 'basepress_general_settings' );
			...
			if ( ! is_multisite() ) {
				add_settings_field( 'remove_all_uninstall', __( 'Remove all content on uninstall', 'basepress' ), array( $this, 'remove_all_uninstall_render' ), 'basepress', 'basepress_general_settings' );
			}
			...
			add_settings_field( 'sections_view_all_text', __( '"View all articles" link text', 'basepress' ), array( $this, 'sections_view_all_text_render' ), 'basepress', 'basepress_theme_settings' );
			...
			add_settings_field( 'posts_orderby', __( 'Articles Order', 'basepress' ), array( $this, 'posts_orderby_render' ), 'basepress', 'basepress_theme_settings' );

			//Add settings fields for BREADCRUMBS settings
			add_settings_field( 'breadcrumbs_kb_name', __( 'Breadcrumbs name', 'basepress' ), array( $this, 'breadcrumbs_kb_name_render' ), 'basepress', 'basepress_breadcrumbs_settings' );
			add_settings_field( 'breadcrumbs_include_home', __( 'Include Home link', 'basepress' ), array( $this, 'breadcrumbs_include_home_render' ), 'basepress', 'basepress_breadcrumbs_settings' );
			add_settings_field( 'breadcrumbs_home_text', __( 'Text for Home link', 'basepress' ), array( $this, 'breadcrumbs_home_text_render' ), 'basepress', 'basepress_breadcrumbs_settings' );
			add_settings_field( 'breadcrumbs_include_parent', __( 'Include parent pages link', 'basepress' ), array( $this, 'breadcrumbs_include_parent_render' ), 'basepress', 'basepress_breadcrumbs_settings' );

			//Add settings fields for SEARCH settings
			add_settings_field( 'show_search_suggest', __( 'Enable live search results', 'basepress' ), array( $this, 'show_search_suggest_render' ), 'basepress', 'basepress_search_settings' );
			add_settings_field( 'min_search_suggest_screen', __( 'Disable live search results on devices smaller than', 'basepress' ), array( $this, 'min_search_suggest_screen_render' ), 'basepress', 'basepress_search_settings' ); //Since version 1.2.0
			add_settings_field( 'search_suggest_count', __( 'Max live search results', 'basepress' ), array( $this, 'search_suggest_count_render' ), 'basepress', 'basepress_search_settings' );
			add_settings_field( 'search_min_chars', __( 'Minimum word length', 'basepress' ), array( $this, 'search_min_chars_render' ), 'basepress', 'basepress_search_settings' );
			add_settings_field( 'search_disable_word_boundary', __( 'Enable wildcard before search terms', 'basepress' ), array( $this, 'search_disable_word_boundary_render' ), 'basepress', 'basepress_search_settings' );
			...
			add_settings_field( 'smartsearch_no_results_message', __( "'No search result found' message for live search", 'basepress' ), array( $this, 'smartsearch_no_results_message_render' ), 'basepress', 'basepress_search_settings' );
			...
			add_settings_field( 'searchbar_style', __( 'Load css with shortcode', 'basepress' ), array( $this, 'searchbar_style_render' ), 'basepress', 'basepress_search_settings' );
			...
			add_settings_field( 'settings_import', __( 'Import Settings', 'basepress' ), array( $this, 'settings_import_render' ), 'basepress', 'basepress_settings_import_export' );
			add_settings_field( 'settings_export', __( 'Export Settings', 'basepress' ), array( $this, 'settings_export_render' ), 'basepress', 'basepress_settings_import_export' );
		...
		*/
		...
		public function article_permalink_structure_render(){
			...
			echo '<input type="text" name="basepress_settings[article_permalink_structure]" value="' . $value . '">';
			echo '/' . __( 'article-slug', 'basepress' ) . '/';
			echo '<p class="description">' . __( 'Available tags are <code>%knowledge_base%</code>, <code>%parent_sections%</code>, <code>%article_section%</code> or <code>%none%</code>.<br>Add a forward slash between each tag. If left empty it defaults to <code>%knowledgebase%/$article_section%</code>.', 'basepress' ) . '</p>';
		}
		...
		public function section_permalink_structure_render(){
			...
			echo '<input type="text" name="basepress_settings[section_permalink_structure]" value="' . $value . '">';
			echo '/' . __( 'section-slug', 'basepress' ) . '/';
			echo '<p class="description">' . __( 'Available tags are <code>%knowledge_base%</code>, <code>%parent_sections%</code> or <code>%none%</code>.<br>Add a forward slash between each tag. If left empty it defaults to <code>%knowledgebase%</code>.', 'basepress' ) . '</p>';
		}
		...
		*/
		...
		public function theme_style_render() {
			...
			foreach ( $themes as $theme ) {
				...
				if ( ! in_array( $theme_dir, $unique_themes ) ) {
					...
					$theme_css = file_get_contents( $style_css, false, null, 0, 200 );
			...
			echo '</select>';
			echo '<p class="description">' . __( 'After saving the settings, further customizations for the selected theme are available under the BasePress menu.', 'basepress' ) . '</p>';
		}
		...
		public function sections_view_all_text_render(){
			...
			echo '<input type="text" name="basepress_settings[sections_view_all_text]" value="' . $value . '">';
			echo '<p class="description">' . __( 'The text appears under the list of articles in the section. Type the singular and plural form for "View all articles" separated by a vertical bar "|".<br>Use %number% to show the number of articles.<br>Default: View %number% article | View all %number% articles.', 'basepress' ) . '</p>';
		}
		...
		public function posts_orderby_render(){
			...
			echo '<option value ="date_asc" ' . selected( 'date_asc', $value, false ) . '>' . __( 'Date ascending', 'basepress' ) . '</option>';
			echo '<option value="date_desc" ' . selected( 'date_desc', $value, false ) . '>' . __( 'Date descending', 'basepress' ) . '</option>';
			echo '<option value="alpha_asc" ' . selected( 'alpha_asc', $value, false ) . '>' . __( 'Alphabetical ascending', 'basepress' ) . '</option>';
			echo '<option value="alpha_desc" ' . selected( 'alpha_desc', $value, false ) . '>' . __( 'Alphabetical descending', 'basepress' ) . '</option>';
			echo '</select>';
		}
		...
		public function breadcrumbs_kb_name_render() {
			...
			echo '<input type="text" name="basepress_settings[breadcrumbs_kb_name]" value="' . $name . '">';
		...
		public function breadcrumbs_home_text_render(){
			...
			echo '<input type="text" name="basepress_settings[breadcrumbs_home_text]" value="' . $name . '">';
			echo '<p class="description">' . __( 'This is the name used in the breadcrumbs for the Home page. Default is Home.', 'basepress' ) . '</p>';
		}
		...
		*/
		...
		public function show_search_suggest_render() {
			...
			echo '<p class="description">' . __( 'Display live search results while typing on the search bar.', 'basepress' ) . '</p>';
		}
		...
		*/
		...
		public function search_suggest_count_render() {
			...
			echo '<p class="description">' . __( 'Maximum number of results to display during a live search.', 'basepress' ) . '</p>';
		}
		...
		public function search_min_chars_render(){
			...
			echo '<input type="number" name="basepress_settings[search_min_chars]" value="' . $value . '" min="1" max="9">';
			echo '<p class="description">' . __( 'Any search term shorter than this value will be omitted from the search.', 'basepress' ) . '</p>';
		}
		...
		*/
		public  function searchbar_style_render() {
			...
			echo '<p class="description">' . __( 'You can add a search bar anywhere in your website using the shortcode [basepress-search].<br>Check this option to load the CSS style for the search bar or leave it unchecked to use your own CSS style.', 'basepress' ) . '</p>';
		}
		...
		*/
		...
		public function settings_import_export_notice_render(){
			echo '<p class="description">' . __( 'Here you can transfer the BasePress settings. If you need to migrate the whole knowledge base content please use the', 'basepress' ) . ' <a href="https://wordpress.org/plugins/basepress-migration-tools/" target="_blank">BasePress Migration tools</p>';
		}

		public function settings_import_render(){
			?>
			<textarea name="basepress_settings_import" rows="8" placeholder="<?php _e( 'Paste your settings and click Import. This action cannot be undone.', 'basepress' ); ?>"></textarea>
			<br>
			<button type="submit" class="button button-primary"><?php _e( 'Import', 'basepress' ); ?></button>
		<?php
		}
		...
		*/
		public function basepress_settings_validate( $input ) {
			...
			if( isset( $_REQUEST['basepress_settings_import'] ) && ! empty( $_REQUEST['basepress_settings_import'] ) ){
				$import = json_decode( stripslashes( $_REQUEST['basepress_settings_import'] ), true );
				if( ! empty( $import ) ){
					$input = $import;
				}
			}
