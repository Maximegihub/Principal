...
if ( ! class_exists( 'Basepress_Manual' ) ) {
	...
	class Basepress_Manual {
		...
		public function add_manual_page() {
			add_submenu_page( 'basepress', 'BasePress ' . __( 'Quick Start Guide', 'basepress' ), __( 'Quick Start Guide', 'basepress' ), 'manage_options', 'basepress_manual', array( $this, 'display_manual_screen' ) );
		}
				...
				<div style="margin:1em 0 0;padding:30px;background-color:#fff;box-shadow:0 1px 1px 0 rgba(0,0,0,.1);">
					<table>
						<tr>
							...
							<td><h1 style="font-size: 3em;"><?php _e( 'Welcome to BasePress', 'basepress' ); ?> <span style="font-size: 0.5em;color:#888;"><?php echo BASEPRESS_VER; ?></span></h1></td>
						</tr>
						<tr>
							<td colspan="2">
								<?php
								if( get_option( 'basepress_run_wizard' ) ){ ?>
									...
									<a class="button" href="<?php echo add_query_arg( 'basepress_skip_wizard', 'true', menu_page_url( 'basepress_wizard', false ) ); ?>"><?php _e( 'Settings Page', 'basepress' ); ?></a>
								<?php }	?>
							</td>
						</tr>
					</table>
